...
struct computeFluxWeno2D {
    ...
    void operator()(const int i, const int j) const {
        ...
        int ns = (int)cd(0);
        double ur,vr,w,b1,b2,b3,w1,w2,w3,p1,p2,p3,f1,f2,f3,f4,f5,a1,a2;
        double dx = cd(1);
        double dy = cd(2);
        ...
        for (int idx=0; idx<2; ++idx){
            ...
            a1 = f1-2.0*f2+f3; a2 = f1-4.0*f2+3.0*f3;
            b1 = (13.0/12.0)*a1*a1 + (0.25)*a2*a2;
            a1 = f2-2.0*f3+f4; a2 = f2-f4;
            b2 = (13.0/12.0)*a1*a1 + (0.25)*a2*a2;
            a1 = f3-2.0*f4+f5; a2 = 3.0*f3-4.0*f4+f5;
...
struct computeFluxQuick2D {
    ...
    computeFluxQuick2D (FS4D var_, FS2D p_, FS2D rho_,
                         FS2D fluxx_, FS2D fluxy_, Kokkos::View<double*> cd_, int v_)
                         : var(var_), p(p_), rho(rho_),
                           fluxx(fluxx_), fluxy(fluxy_), cd(cd_), v(v_) {}
...
struct calculateWenoFluxesG {
    ...
    calculateWenoFluxesG (FS4D var_, FS3D p_, FS3D rho_, FS4D u_,
                         FS3D wenox_, FS3D wenoy_, FS3D wenoz_, Kokkos::View<double*> cd_, int v_)
                         : var(var_), p(p_), rho(rho_), vel(u_),
    ...
    void operator()(const int i, const int j, const int k) const {
        ...
        int ns = (int)cd(0);
        double ur,vr,wr,w,b1,b2,b3,w1,w2,w3,p1,p2,p3,f1,f2,f3,f4,f5,a1,a2;
