...
inline
void cart3d_func::pushRegion(std::string name, bool saveDvar){
  if (cf.diagnostics){ 
    if (saveDvar) dg.start(cf.t,dvar);
    timers[name].reset();
  }
}
