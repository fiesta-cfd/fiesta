...
struct calculateRhoAndPressure {
  ...
  calculateRhoAndPressure(FS4D var_, FS3D p_, FS3D rho_,
...
struct calculateWenoFluxes {
  ...
  calculateWenoFluxes(FS4D var_, FS3D p_, FS3D rho_, FS3D wenox_, FS3D wenoy_,
                      FS3D wenoz_, Kokkos::View<double *> cd_, int v_)
      : var(var_), p(p_), rho(rho_), wenox(wenox_), wenoy(wenoy_),
  ...
  void operator()(const int i, const int j, const int k) const {
    ...
    ur = (-var(i + 2, j, k, 0) / rho(i + 2, j, k) +
          7.0 * var(i + 1, j, k, 0) / rho(i + 1, j, k) +
          7.0 * var(i, j, k, 0) / rho(i, j, k) -
          var(i - 1, j, k, 0) / rho(i - 1, j, k)) /
         12.0;

    vr = (-var(i, j + 2, k, 1) / rho(i, j + 2, k) +
          7.0 * var(i, j + 1, k, 1) / rho(i, j + 1, k) +
          7.0 * var(i, j, k, 1) / rho(i, j, k) -
          var(i, j - 1, k, 1) / rho(i, j - 1, k)) /
         12.0;

    wr = (-var(i, j, k + 2, 2) / rho(i, j, k + 2) +
          7.0 * var(i, j, k + 1, 2) / rho(i, j, k + 1) +
          7.0 * var(i, j, k, 2) / rho(i, j, k) -
          var(i, j, k - 1, 2) / rho(i, j, k - 1)) /
         12.0;
...
hydroc3d_func::hydroc3d_func(struct inputConfig &cf_,
                             Kokkos::View<double *> &cd_)
    : rk_func(cf_, cd_) {

  grid = Kokkos::View<double ****, FS_LAYOUT>("coords", cf.ni, cf.nj, cf.nk, 3);
  var = Kokkos::View<double ****, FS_LAYOUT>("var", cf.ngi, cf.ngj, cf.ngk,
                                             cf.nvt); // Primary Variable Array
  tmp1 = Kokkos::View<double ****, FS_LAYOUT>(
      "tmp1", cf.ngi, cf.ngj, cf.ngk, cf.nvt); // Temporary Variable Arrayr1
  // tmp2    = Kokkos::View<double****  ,FS_LAYOUT>("tmp2",
  // cf.ngi,cf.ngj,cf.ngk,cf.nvt); // Temporary Variable Array2
  dvar = Kokkos::View<double ****, FS_LAYOUT>("dvar", cf.ngi, cf.ngj, cf.ngk,
                                              cf.nvt); // RHS Output
  p = Kokkos::View<double ***, FS_LAYOUT>("p", cf.ngi, cf.ngj,
                                          cf.ngk); // Pressure
  T = Kokkos::View<double ***, FS_LAYOUT>("T", cf.ngi, cf.ngj,
                                          cf.ngk); // Temperature
  rho = Kokkos::View<double ***, FS_LAYOUT>("rho", cf.ngi, cf.ngj,
                                            cf.ngk); // Total Density
  qx = Kokkos::View<double ***, FS_LAYOUT>(
      "qx", cf.ngi, cf.ngj, cf.ngk); // Heat Fluxes in X direction
  qy = Kokkos::View<double ***, FS_LAYOUT>(
      "qy", cf.ngi, cf.ngj, cf.ngk); // Heat Fluxes in X direction
  qz = Kokkos::View<double ***, FS_LAYOUT>(
      "qz", cf.ngi, cf.ngj, cf.ngk); // Heat Fluxes in z direction
  fluxx = Kokkos::View<double ***, FS_LAYOUT>(
      "fluxx", cf.ngi, cf.ngj, cf.ngk); // Advective Fluxes in X direction
  fluxy = Kokkos::View<double ***, FS_LAYOUT>(
      "fluxy", cf.ngi, cf.ngj, cf.ngk); // Advective Fluxes in Y direction
  fluxz = Kokkos::View<double ***, FS_LAYOUT>(
      "fluxz", cf.ngi, cf.ngj, cf.ngk); // Advective Fluxes in z direction
  stressx = Kokkos::View<double *****, FS_LAYOUT>(
      "stressx", cf.ngi, cf.ngj, cf.ngk, 3, 3); // stress tensor on x faces
  stressy = Kokkos::View<double *****, FS_LAYOUT>(
      "stressy", cf.ngi, cf.ngj, cf.ngk, 3, 3); // stress tensor on y faces
  stressz = Kokkos::View<double *****, FS_LAYOUT>(
      "stressz", cf.ngi, cf.ngj, cf.ngk, 3, 3); // stress tensor on z faces
  gradRho = Kokkos::View<double ****, FS_LAYOUT>(
      "gradRho", cf.ngi, cf.ngj, cf.ngk, 5); // density gradient vector
  cFlux = Kokkos::View<double ****, FS_LAYOUT>("cFlux", cf.ngi, cf.ngj, cf.ngk,
                                               3); //
...
void hydroc3d_func::compute() {
  ...
  for (int v = 0; v < cf.nv; ++v) {
    timers["flux"].reset();
    Kokkos::parallel_for(
        weno_pol, calculateWenoFluxes(var, p, rho, fluxx, fluxy, fluxz, cd, v));

    Kokkos::parallel_for(cell_pol,
                         applyWenoFluxes(dvar, fluxx, fluxy, fluxz, v));
    Kokkos::fence();
