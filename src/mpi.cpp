...
void haloExchange(struct inputConfig cf, Kokkos::View<double**> &deviceV, class mpiBuffers &m){
    ...
    Kokkos::parallel_for( xPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
        ...
        m.leftSend(idx_bc,v) = deviceV(idxl,v);
        m.rightSend(idx_bc,v) = deviceV(idxr,v);
    });
    Kokkos::parallel_for( yPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
        ...
        m.bottomSend(idx_bc,v) = deviceV(idxb,v);
        m.topSend(idx_bc,v) = deviceV(idxt,v);
    });
    if (cf.ndim == 3){
        Kokkos::parallel_for( zPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
            ...
            m.backSend(idx_bc,v) = deviceV(idxb,v);
            m.frontSend(idx_bc,v) = deviceV(idxf,v);
        });
    }
    ...
    Kokkos::parallel_for( xPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
        ...
        int idx_bc = i + j * cf.ng + k * cf.ng * cf.ngj;
        deviceV(idxl,v) = m.leftRecv(idx_bc,v);
        deviceV(idxr,v) = m.rightRecv(idx_bc,v);
    });
    Kokkos::parallel_for( yPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
        int idxb   = i + j * cf.ngi + k * cf.ngi * cf.ngj;
        int idxt   = i + (cf.ngj - cf.ng + j) * cf.ngi + k * cf.ngi * cf.ngj;
        int idx_bc = i + j * cf.ng + k * cf.ng * cf.ngj;
        deviceV(idxb,v) = m.bottomRecv(idx_bc,v);
        deviceV(idxt,v) = m.topRecv(idx_bc,v);
    });
    if (cf.ndim == 3){
        Kokkos::parallel_for( zPol, KOKKOS_LAMBDA  (const int i, const int j, const int k, const int v){
            int idxb   = i + j * cf.ngi + k * cf.ngi * cf.ngj;
            ...
            int idx_bc = i + j * cf.ng + k * cf.ng * cf.ngj;
            deviceV(idxb,v) = m.backRecv(idx_bc,v);
            deviceV(idxf,v) = m.frontRecv(idx_bc,v);
        });
    }
}
