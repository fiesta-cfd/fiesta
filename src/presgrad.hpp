...
struct applyPressureGradient3D {
  ...
  void operator()(const int i, const int j, const int k) const {
    ...
    // calculate pressure gradient across cell in each direction using 4th order
    // central difference
    double dxp = (p(i-2,j,k) - 8.0*p(i-1,j,k) + 8.0*p(i+1,j,k) - p(i+2,j,k))/(12.0*dx);
    double dyp = (p(i,j-2,k) - 8.0*p(i,j-1,k) + 8.0*p(i,j+1,k) - p(i,j+2,k))/(12.0*dy);
    double dzp = (p(i,j,k-2) - 8.0*p(i,j,k-1) + 8.0*p(i,j,k+1) - p(i,j,k+2))/(12.0*dz);
