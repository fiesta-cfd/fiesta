...
void cgnsWriter::writeGrid(struct inputConfig cf, const FS4D gridD,
                           const char *fname) {
...
void cgnsWriter::writeSPGrid(struct inputConfig cf, const FS4D gridD,
                             const char *fname) {
...
void cgnsWriter::writeRestart(struct inputConfig cf, const FS4D gridD,
                              const FS4D varD, int tdx, double time) {
  ...
  if (cf.rank == 0) {
    cout << c(0, YEL) << left << setw(22)
         << "    Writing Restart: " << c(0, NON) << c(0, CYA) << left
         << "'" + fsname + "'" << c(0, NON) << endl;
  }
  ...
  if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealDouble, "MomentumX",
                      &index_flow))
    cgp_error_exit();

  if (cgp_field_write_data(fIdx, bIdx, zIdx, index_sol, index_flow, start, endc,
                           v))
    cgp_error_exit();
  ...
  if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealDouble, "MomentumY",
                      &index_flow))
    cgp_error_exit();
  ...
  if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealDouble, "MomentumZ",
                      &index_flow))
    cgp_error_exit();
  ...
  if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealDouble,
                      "EnergyInternal", &index_flow))
    cgp_error_exit();
  ...
  for (int vn = cf.ndim + 1; vn < cf.nv; ++vn) {
    ...
    if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealDouble, dName,
                        &index_flow))
      cgp_error_exit();

    if (cgp_field_write_data(fIdx, bIdx, zIdx, index_sol, index_flow, start,
                             endc, v))
      cgp_error_exit();
  }
...
void cgnsWriter::writeSolution(struct inputConfig cf, const FS4D gridD,
                               const FS4D varD, int tdx, double time) {
  int index_sol, index_flow;
  ...
  for (int vn = 0; vn < cf.nvt; ++vn) {
    ...
    if (cgp_field_write(fIdx, bIdx, zIdx, index_sol, CG_RealSingle, dName,
                        &index_flow))
      cgp_error_exit();
  ...
  // if
  // (cgp_field_write(fIdx,bIdx,zIdx,index_sol,CG_RealSingle,"MomentumX",&index_flow))
  //    cgp_error_exit();

  // if
  // (cgp_field_write_data(fIdx,bIdx,zIdx,index_sol,index_flow,start,endc,vsp))
  //    cgp_error_exit();
  ...
  //
  // if
  // (cgp_field_write(fIdx,bIdx,zIdx,index_sol,CG_RealSingle,"MomentumY",&index_flow))
  //    cgp_error_exit();
  ...
  //
  // if
  // (cgp_field_write(fIdx,bIdx,zIdx,index_sol,CG_RealSingle,"MomentumZ",&index_flow))
  //    cgp_error_exit();
  ...
  //
  // if
  // (cgp_field_write(fIdx,bIdx,zIdx,index_sol,CG_RealSingle,"EnergyInternal",&index_flow))
  //    cgp_error_exit();
  ...
  //
  //    if
  //    (cgp_field_write(fIdx,bIdx,zIdx,index_sol,CG_RealSingle,dName,&index_flow))
  //        cgp_error_exit();
