...
void applyBCs(struct inputConfig cf, Kokkos::View<double****> &u){
    ...
    if (cf.xMinus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngj,cf.ngk,cf.nv+4}), bc_L(cf.ng,cf.ng,u));
    }

    if (cf.xPlus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngj,cf.ngk,cf.nv+4}), bc_R(cf.ng+cf.nci,cf.ng,u));
    }

    if (cf.yMinus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngi,cf.ngk,cf.nv+4}), bc_B(cf.ng,cf.ng,u));
    }

    if (cf.yPlus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngi,cf.ngk,cf.nv+4}), bc_T(cf.ng+cf.ncj,cf.ng,u));
    }

    if (cf.zMinus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngi,cf.ngj,cf.nv+4}), bc_H(cf.ng,cf.ng,u));
    }

    if (cf.zPlus < 0){
        Kokkos::parallel_for(policy_bl({0,0,0},{cf.ngi,cf.ngj,cf.nv+4}), bc_F(cf.ng+cf.nck,cf.ng,u));
    }
}
